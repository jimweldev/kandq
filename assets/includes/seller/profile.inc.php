	...
	while ($row = mysqli_fetch_assoc($result)) {
		...
		if (password_verify($password, $row["password"])) {
			...
			if (empty($_FILES['image']['tmp_name'])) {
				...
				header("Location: " . $baseUrl . "seller/profile/?success");
				exit();
		...
		header("Location: " . $baseUrl . "seller/profile/?error=Incorrect password");
		exit();
...
if (isset($_POST["updateAddress"])) {
	...
	while ($row = mysqli_fetch_assoc($result)) {
		...
		if (password_verify($password, $row["password"])) {
			...
			header("Location: " . $baseUrl . "seller/profile/complete-address?success");
			exit();

		}

		header("Location: " . $baseUrl . "seller/profile/complete-address?error=Incorrect password");
		exit();
...
if (isset($_POST["updatePassword"])) {
	...
	if ($newPassword != $confirmPassword) {
		...
		header("Location: " . $baseUrl . "seller/profile/change-password?error=Passwords Mismatch");
		exit();
	...
	while ($row = mysqli_fetch_assoc($result)) {
		...
		if (password_verify($oldPassword, $row["password"])) {
			...
			header("Location: " . $baseUrl . "seller/profile/change-password?success");
			exit();

		}

		header("Location: " . $baseUrl . "seller/profile/change-password?error=Incorrect password");
		exit();
